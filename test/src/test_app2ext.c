...
int app_install_gdbus()
{
	...
	if (uid == OWNER_ROOT || uid == GLOBAL_USER) {
		printf("Not supported user(%d), local user only", uid);
		return -1;
	}

	printf("app_install_gdbus : path(%s)\n", TEST_PKGNAME_PATH);
	...
	printf("launch application after install");
...
int app_install()
{
	...
	*/
	ret = handle->interface.client_pre_install(TEST_PKGNAME, dir_list, 20);
	if (ret) {
