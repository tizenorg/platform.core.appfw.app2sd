...
static void usage(void)
{
	...
	printf("------ INSTALL/UNINSTALL/UPGRADE TEST ----------\n");
	printf("1.$test_app2ext --pre_install\n");
	printf("2.$pkgcmd -it tpk {pkg_path} (--global)\n");
	printf("3.$test_app2ext --post_uninstall\n");
	printf("------------------------------------------------\n");
	printf("------ MOVE PKG TEST ---------------------------\n");
	printf("$test_app2ext --move\n");
	printf("------ GET INSTALLED LOCATION (Ext/Internal) ---\n");
	printf("$test_app2ext --getlocation\n");
	printf("------------------------------------------------\n");
	printf("------ ENABLE/DISABLE TEST W/ Installed PKG ----\n");
...
#define OPTVAL_ENABLE_EXTERNAL_DIR	1010
#define OPTVAL_DISABLE_EXTERNAL_DIR	1011
#define OPTVAL_USAGE			1012

/* Supported options */
const struct option long_opts[] = {
        { "pre_install", 0, NULL, OPTVAL_PRE_INSTALL },
        { "post_install", 0, NULL, OPTVAL_POST_INSTALL },
	{ "pre_uninstall", 0, NULL, OPTVAL_PRE_UNINSTALL },
	{ "post_uninstall", 0, NULL, OPTVAL_POST_UNINSTALL },
	{ "pre_upgrade", 0, NULL, OPTVAL_PRE_UPGRADE },
	{ "post_upgrade", 0, NULL, OPTVAL_POST_UPGRADE },
	...
	//{ "enabledir", 0, NULL, OPTVAL_ENABLE_EXTERNAL_DIR },
	//{ "disabledir", 0, NULL, OPTVAL_DISABLE_EXTERNAL_DIR },
...
static int app_disable()
{
	...
	if (ret < 0 || ret > 47) {
		printf("app_disable failed : unknown error\n");
	} else {
		printf("app_disable return(%s)\n", error_list[ret]);
	}
...
static int pre_app_upgrade()
{
	...
	printf("app_upgrade_client for [%s]\n", TEST_PKGNAME);
...
static void enable_external_dir()
{
	printf("enable_external_dir_client\n");
	int ret = -1;

	ret = app2ext_usr_enable_external_dir(getuid());
	if (ret == 0) {
		printf("success\n");
	} else {
		printf("failed\n");
...
static void disable_external_dir()
{
	printf("disable_external_dir\n");
	int ret = -1;

	ret = app2ext_usr_disable_external_dir(getuid());
	if (ret == 0) {
		printf("success\n");
	} else {
		printf("failed\n");
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		case OPTVAL_DISABLE_APP:
			app_disable();
			break;
		case OPTVAL_ENABLE_EXTERNAL_DIR:
			enable_external_dir();
			break;
		case OPTVAL_DISABLE_EXTERNAL_DIR:
