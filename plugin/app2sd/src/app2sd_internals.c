		...
		uid_t uid, char **dev_node)
{
	...
	snprintf(dmcrypt_open_cmd, BUF_SIZE, "echo '%s' | cryptsetup -q luksOpen %s %s",
		passwd, loopback_device, pkgid);
	_D("cmd (%s)", dmcrypt_open_cmd);
	...
	*dev_node = (char *)calloc(len + 1, sizeof(char));
	...
	snprintf(*dev_node, len + 1, "/dev/mapper/%s", pkgid);
...
int _app2sd_dmcrypt_close_device(const char *loopback_device)
{
	...
	t_dev_node =
		_app2sd_find_associated_dmcrypt_device_node(loopback_device);
	if (!t_dev_node) {
		_E("no associated device node found for (%s)",
			loopback_device);
	...
	snprintf(dev_node, BUF_SIZE, "/dev/mapper/%s", loopback_device);
...
char *_app2sd_find_associated_dmcrypt_device_node(const char *loopback_device)
{
	...
	if (asprintf(&dev_node, "/dev/mapper/%s", loopback_device) == 0) {
		...
		const char *temp_loopback_device, uid_t uid)
{
	...
	ret = _app2sd_dmcrypt_open_device(pkgid, temp_loopback_device,
		uid, &device_node);
	if (ret) {
...
#ifdef _APPFW_FEATURE_APP2SD_DMCRYPT_ENCRYPTION
	ret = _app2sd_dmcrypt_close_device(loopback_device);
	if (ret)
...
FINISH_OFF:
	if (dev_node) {
#ifdef _APPFW_FEATURE_APP2SD_DMCRYPT_ENCRYPTION
		ret = _app2sd_dmcrypt_close_device(temp_loopback_device);
		if (ret)
			_E("close dmcrypt device error(%d)", ret);
#else
