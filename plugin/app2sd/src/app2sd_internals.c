...
static int _app2sd_setup_path(const char* path, const char *label)
{
...
static int _app2sd_apply_app_smack(const char *application_path,
		const char *pkgid, GList* dir_list)
{
	int ret = APP2EXT_SUCCESS;
	GList *list = NULL;
	...
	while (list) {
		...
		if (dir_detail && dir_detail->name
			...
			_D("set label(%s) to path(%s)",
				label, temp_dir_path);
			ret = _app2sd_setup_path(temp_dir_path, label);
			if (ret) {
...
char *_app2sd_find_associated_device_node(const char *loopback_device)
{
	...
	if (result == NULL) {
		_E("there is no the associated file (%s)", loopback_device);
		return NULL;
	}
					 ...
					 const char *dir_name)
{
	...
	if ((ret = symlink(application_dir_mmc_path,
		...
		if (errno == EEXIST) {
			_E("file with symlink name present (%s)",
...
int _app2sd_mount_app_content(const char *application_path, const char *pkgid,
		const char *dev, int mount_type, GList* dir_list, app2sd_cmd cmd)
{
	int ret = APP2EXT_SUCCESS;
	mode_t mode = DIR_PERMS;
	...
	if (ret) {
		...
		snprintf(label, FILENAME_MAX, "User::Pkg::%s::RO", pkgid);
		ret = _app2sd_setup_path(application_mmc_path, label);
		if (ret) {
...
int _app2sd_unmount_app_content(const char *application_path)
{
	...
	if ((ret = umount(application_dir_mmc_path)) < 0) {
		strerror_r(errno, err_buf, sizeof(err_buf));
		_E("unable to umount the dir (%s)", err_buf);
	}
...
int _app2sd_move_app_to_external(const char *pkgid, GList* dir_list, uid_t uid)
{
	...
	ret = _app2sd_apply_app_smack(application_path, pkgid, dir_list);
	if (ret) {
		...
		GList* dir_list, char *dev_node, int size)
{
	...
	_D("App2Sd  : duplicate setup SUCCESS");
	...
	ret = _app2sd_mount_app_content(temp_application_path, pkgid,
		dev_node, MOUNT_TYPE_RW, dir_list, APP2SD_PRE_UPGRADE);
	if (ret) {
		_E("remount failed");
		...
		int size, GList* dir_list)
{
	...
	if (NULL == old_device_node) {
		...
		ret = _app2sd_mount_app_content(application_path, pkgid,
			old_device_node, MOUNT_TYPE_RW, dir_list, APP2SD_PRE_UPGRADE);
		if (ret) {
			_E("remount failed");
			err_res = APP2EXT_ERROR_MOUNT_PATH;
		}
		...
		/* do re-mounting */
		ret = _app2sd_mount_app_content(application_path, pkgid,
			old_device_node, MOUNT_TYPE_RW_REMOUNT, dir_list, APP2SD_PRE_UPGRADE);
		if (ret) {
			_E("remount failed");
			err_res = APP2EXT_ERROR_MOUNT_PATH;
		}
