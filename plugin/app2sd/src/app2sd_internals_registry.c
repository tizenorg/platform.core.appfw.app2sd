...
static int _app2sd_check_existing_info(const char *pkgid,
		const char *loopback_device, uid_t uid)
{
	...
	query = sqlite3_mprintf("select count(*) from app2sd_info " \
		"where pkgid=%Q and filename=%Q and uid=%d",
		pkgid, loopback_device, uid);

	ret = sqlite3_prepare_v2(app2sd_db, query, strlen(query), &stmt, NULL);
	if (ret != SQLITE_OK) {
		LOGE("prepare failed: %s", sqlite3_errmsg(app2sd_db));
		sqlite3_free(query);
		return -1;
	}
		...
		const char *loopback_device, uid_t uid)
{
	...
	ret = _app2sd_check_existing_info(pkgid, loopback_device, uid);
	if (ret < 0) {
	}
...
int _app2sd_get_info_from_db(const char *filename, char **pkgid, uid_t *uid)
{
	...
	if (SQLITE_OK != sqlite3_prepare_v2(app2sd_db, query,
		strlen(query), &stmt, NULL)) {
		_E("sqlite3 prepare error (%s)", sqlite3_errmsg(app2sd_db));
		sqlite3_free(query);
		return APP2EXT_ERROR_SQLITE_REGISTRY;
	}
