...
int app2sd_usr_pre_app_install(const char *pkgid, GList* dir_list, int size, uid_t uid)
{
	...
	if (_is_global(uid)) {
		...
		snprintf(loopback_device, FILENAME_MAX - 1, "%s/%d/%s",
			APP2SD_PATH, uid, encoded_id);
		tzplatform_reset_user();
	}
		...
		int size, uid_t uid)
{
	...
	if ((int)curr_size < reqd_disk_size) {
		...
		temp_pkgid = calloc(len, sizeof(char));
		if (temp_pkgid == NULL) {
		...
		if (_is_global(uid)) {
			...
			temp_application_path = calloc(len, sizeof(char));
			...
			temp_loopback_device = calloc(len, sizeof(char));
			...
			snprintf(temp_loopback_device, len, "%s/%d/%s",
				APP2SD_PATH, uid, temp_encoded_id);
			free(temp_encoded_id);
			tzplatform_reset_user();
		}
