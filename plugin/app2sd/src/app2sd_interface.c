...
int app2sd_usr_pre_app_install(const char *pkgid, GList* dir_list, int size, uid_t uid)
{
	...
	if (result != NULL) {
		app2ext_print("App2Sd Error : there is "
			"same associated File (%s)\n", loopback_device);
	...
	ret =_app2sd_mount_app_content(application_path, pkgid,
		device_node, MOUNT_TYPE_RW, dir_list, APP2SD_PRE_INSTALL);
	if (ret) {
		app2ext_print("App2Sd Error : "
...
int app2sd_usr_on_demand_setup_init(const char *pkgid, uid_t uid)
{
	...
	/* do mounting */
	ret = _app2sd_mount_app_content(application_path, pkgid,
		device_node, MOUNT_TYPE_RD, NULL, APP2SD_APP_LAUNCH);
	if (ret) {
		...
		app2ext_move_type move_type, uid_t uid)
{
	...
	pkgmgrinfo_ret = pkgmgrinfo_pkginfo_get_usr_pkginfo(pkgid, uid, &info_handle);
	if (pkgmgrinfo_ret < 0) {
		_E("failed to get pkginfo for pkg(%s), uid(%d)", pkgid, uid);
	}
	pkgmgrinfo_ret = pkgmgrinfo_pkginfo_get_installed_storage(info_handle, &storage);
	if (pkgmgrinfo_ret < 0) {
		_E("failed to get installed storage for pkg(%s) of uid(%d)", pkgid, uid);
	...
	if (move_type == APP2EXT_MOVE_TO_EXT) {
		pkgmgrinfo_ret =
			pkgmgrinfo_pkginfo_set_usr_installed_storage(pkgid,
				INSTALL_EXTERNAL, uid);
		if (pkgmgrinfo_ret < 0) {
		...
		int size, uid_t uid)
{
	...
	if (NULL == device_node) {
		...
		/* do mounting */
		ret = _app2sd_mount_app_content(application_path, pkgid,
			device_node, MOUNT_TYPE_RW, dir_list, APP2SD_PRE_UPGRADE);
		if (ret) {
			app2ext_print("App2Sd Error : Re-mount failed\n");
			if (device_node) {
				free(device_node);
		...
		/* do re-mounting */
		ret = _app2sd_mount_app_content(application_path, pkgid,
			device_node, MOUNT_TYPE_RW_REMOUNT, NULL, APP2SD_PRE_UPGRADE);
		if (ret) {
			app2ext_print("App2Sd Error : Re-mount failed\n");
			if (device_node) {
				free(device_node);
