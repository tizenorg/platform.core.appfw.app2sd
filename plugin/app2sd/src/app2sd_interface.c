		...
		app2ext_move_type move_type, uid_t uid)
{
	...
	if (move_type == APP2EXT_MOVE_TO_EXT) {
		pkgmgrinfo_ret = pkgmgrinfo_pkginfo_set_usr_installed_storage(pkgid, INSTALL_EXTERNAL, uid);
		if (pkgmgrinfo_ret < 0) {
			_E("failed to update installed location to db [%s, %s] of uid(%d)",
				pkgid, INSTALL_EXTERNAL, uid);
		}
	} else {
		pkgmgrinfo_ret = pkgmgrinfo_pkginfo_set_usr_installed_storage(pkgid, INSTALL_INTERNAL, uid);
		if (pkgmgrinfo_ret < 0) {
			_E("failed to update installed location to db [%s, %s] of uid(%d)",
