	...
	GVariant *parameters, GDBusMethodInvocation *invocation)
{
	...
	ret = app2sd_pre_app_install(pkgid, dir_list, size, sender_uid);
	if (ret) {
		_D("app_install error(%d)", ret);
		result = ret;
	}
	...
	GVariant *parameters, GDBusMethodInvocation *invocation)
{
	...
	ret = app2sd_post_app_install(pkgid, install_status, sender_uid);
	if (ret) {
		_D("post_app_install error(%d)", ret);
		result = ret;
	}
...
static void app2sd_server_pre_app_uninstall(GDBusConnection *connection, const gchar *sender,
	GVariant *parameters, GDBusMethodInvocation *invocation)
{
	GVariant *param = NULL;
	int result = APP2EXT_SUCCESS;
	...
	_D("pkgid(%s), sender_pid(%d), sender_uid(%d)",
		pkgid, sender_pid, sender_uid);

	ret = app2sd_pre_app_uninstall(pkgid, sender_uid);
	if (ret) {
		_D("pre_app_uninstall error(%d)", ret);
		result = ret;
	}
...
static void app2sd_server_post_app_uninstall(GDBusConnection *connection, const gchar *sender,
	GVariant *parameters, GDBusMethodInvocation *invocation)
{
	GVariant *param = NULL;
	int result = APP2EXT_SUCCESS;
	...
	ret = app2sd_post_app_uninstall(pkgid, sender_uid);
	if (ret) {
		_D("post_app_uninstall error(%d)", ret);
		result = ret;
	}
	...
	gpointer user_data)
{
	if (g_strcmp0(method_name, "PreAppInstall") == 0) {
		...
		app2sd_server_pre_app_uninstall(connection, sender, parameters, invocation);
	} else if (g_strcmp0(method_name, "PostAppUninstall") == 0) {
		app2sd_server_post_app_uninstall(connection, sender, parameters, invocation);
	}
